...
public abstract class AbstractServiceListener<T> implements
		PivoServiceListener<T>, Serializable {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(AbstractServiceListener.class);
	...
	public void serviceChanged(ServiceEvent serviceEvent) {
		if (serviceEvent != null) {
			T service = (T) bundleContext.getService(serviceEvent
					.getServiceReference());
			if (service != null) {
				...
				case ServiceEvent.UNREGISTERING:
					...
					break;
				default:
					LOGGER.warn("Le type d'event levé ({})n'est pas géré",
							serviceEvent.getType());
					break;
				}
			} else {
				LOGGER.warn(
						"Le service de type {} n'a pas été trouvé dans le contexte OSGi / FILTRE : [{}] ",
						getServiceType(), getFilter());
			}
	...
	public final void registerItself(BundleContext bundleContext)
		...
		if (bundleContext != null) {
			LOGGER.debug("Enregistrement du service {} dans le bundle context",
					this.getClass().getName());
			bundleContext.addServiceListener(this, this.getFilter());
			this.bundleContext = bundleContext;
		} else {
			LOGGER.error(
					"Le bundle context est vide. Le service {} ne sera pas enregistré.",
					this.getClass().getName());
		}
	}
