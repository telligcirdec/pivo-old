package com.mmdi.projet.pivo.webapp.vaadin.view.component;
...
public class ModuleUiButon extends Button implements Comparable<ModuleUiButon>,
		Button.ClickListener {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(ModuleUiButon.class);
	...
	public ModuleUiButon(final ModuleUi moduleUi,
		...
		this.publisher = eventAdminServiceListener.getPublisher(this,
				TOPIC_NAVIGATOR);
	...
	public int compareTo(ModuleUiButon mainButonModuleUiFactoryToCompare) {
		String moduleUiLibelleToCompare = mainButonModuleUiFactoryToCompare
				.getModuleUi().getLibelle();
		String moduleUiLibelleSource = this.getModuleUi().getLibelle();
		Integer displayOrderToCompare = mainButonModuleUiFactoryToCompare
				.getModuleUi().getDisplayOrder();
		Integer displayOrderSource = this.getModuleUi().getDisplayOrder();
		if (displayOrderToCompare == null
				|| Integer.signum(displayOrderToCompare) < 1) {
			displayOrderToCompare = Integer.MAX_VALUE;
		}
		if (displayOrderSource == null
				|| Integer.signum(displayOrderSource) < 1) {
	...
	public void buttonClick(ClickEvent event) {
		...
		if (!viewUiMap.isEmpty() && viewUiMap.size() == 1) {
			...
			publisher.publishEventSynchronously(NavigatorEventHandler
					.getNavigateEventProperty("container/new/modules/"
							+ this.moduleUi.getCode() + "/views/" + viewCode,
							sessionId));
	...
	private void createSecoundaryButtonVerticalLayoutAndAddIt(
		...
		if (viewUiButtonVl == null) {
			...
			for (String key : keySet) {
				...
				if (viewUi.getVisibleOnMenu()) {
					Button viewUiButton = new Button(viewUi.getLibelle(),
							viewUi.getIcon());
					...
					viewUiButtonVl.setComponentAlignment(viewUiButton,
	...
	public VerticalLayout getVerticalLayoutParent() {
		...
		throw new IllegalStateException(
				"Parent of ModuleUiButon must be a vertical layout");
	}

	private boolean parentIsVerticalLayout() {
		return VerticalLayout.class.isAssignableFrom(this.getParent()
				.getClass());
	}
